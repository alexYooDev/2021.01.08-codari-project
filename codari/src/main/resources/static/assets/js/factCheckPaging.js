...
$(window)
		.scroll(
				function() {
					if (Math.ceil($(window).scrollTop()) >= $(document)
							.height()
							- $(window).height()) {
						if (ajaxSwitch == false) {
							ajaxSwitch = true;
							$(`div#loading`)
									.after(
											`<img id="removeImg" alt="하이요" style="display: block; margin:0 auto !important;" src="/img/viewLoading.gif" /><br><strong><p style="text-align: center" id="removeImg">손 씻기에 동참해주세요!</p></strong>`)
							setTimeout(function() {
								console.log(page + " 번쨰 페이지");
								getList(page);
								page++;
