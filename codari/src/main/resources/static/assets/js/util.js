(function($) {
	...
	$.fn.navList = function() {
		...
		$a
				.each(function() {
					...
					var $this = $(this), indent = Math.max(0, $this
							.parents('li').length - 1), href = $this
							.attr('href'), target = $this.attr('target');
	...
	$.fn.panel = function(userConfig) {
		...
		// Vars.
		var $this = $(this), $body = $('body'), $window = $(window), id = $this
				.attr('id'), config;
		...
		if (config.hideOnClick) {
			...
			$this.on('click', 'a', function(event) {
				...
				var $a = $(this), href = $a.attr('href'), target = $a
						.attr('target');
				...
				.on(
						...
						function(event) {
							...
							var diffX = $this.touchPosX
									- event.originalEvent.touches[0].pageX, diffY = $this.touchPosY
									- event.originalEvent.touches[0].pageY, th = $this
									.outerHeight(), ts = ($this.get(0).scrollHeight - $this
									.scrollTop());
							...
							// Prevent vertical scrolling past the top or
							// bottom.
	 ...
	 * 
	 * @param {jQuery}
	 *            $elements Elements (or selector) to move.
	 * @param {bool}
	 *            condition If true, moves elements to the top. Otherwise, moves
